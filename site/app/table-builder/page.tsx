...
export default function TableBuilder() {
  ...
  async function run(explicit?: URLSearchParams) {
    ...
    try {
      // /api/reproduce returns the exact code for this selection WITHOUT running it
      // (the full archive is download-only) — instant no matter how big the query is.
      const j = await (await fetch("/api/reproduce?" + p.toString())).json();
      if (j.error) { setStatus("Error: " + j.error); return; }
      setRepro(j);
      setStatus("Code ready — run it in Colab or locally");
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <Button size="lg" onClick={() => run()}>Generate code</Button>
        ...
        <Card>
          <CardHeader className="flex flex-row items-start justify-between space-y-0">
            <div>
              ...
              <p className="text-sm text-muted-foreground">Runnable code for the selection above, against the full public dataset. Open it in Colab (no setup) or download the script — run it and you get the table.</p>
            </div>
